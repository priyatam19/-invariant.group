...
def manifest():
    return {"schema_version": 1, "status": "passed", "llvm_version": "18.1.3",
            "pipeline": "default<O2>", "heuristic": "v1", "ir_sha256": "ir",
            "input": {"sha256": "source"}, "source_hashes": {"src/LPTAInstrumentation.cpp": "plugin"},
...
class ExperimentTests(unittest.TestCase):
    def test_repeated_invocations_are_aggregated(self):
        ...
        after["summary"] = experiment.summarize([{"unit_kind": "function", "pass": "P", "ir_changed": True}])
        diff = comparison.compare(before, after)
        self.assertEqual(diff["pass_changes"][0]["delta"], {"events": -1, "changed": 0, "candidates": 0})

    def test_frontend_changes_are_labeled(self):
        before, after = manifest(), manifest()
        after["ir_sha256"] = "new-ir"
        self.assertEqual(comparison.compare(before, after)["comparison"], "same-source-includes-frontend-changes")
    ...
    def test_invalidated_units_need_no_after_snapshot(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "trace.jsonl"
            row = {"seq": 0, "unit_kind": "loop", "pass": "LoopDeletionPass", "unit_name": "f",
                   "invalidated": True, "incremental_update_candidate": False}
