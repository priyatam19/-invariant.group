...
def main():
    ...
    with tempfile.TemporaryDirectory(
        prefix="lpta-smoke-", dir=args.build_dir
    ) as directory:
        out = Path(directory)
        ir = out / "input.ll"
        subprocess.run(
        ...
        if any(
            r["unit_kind"] not in {"function", "loop", "module", "scc"} for r in rows
        ):
        ...
        print(
            f"LLVM {version}: verifier, trace, and instrumentation transparency passed"
        )
