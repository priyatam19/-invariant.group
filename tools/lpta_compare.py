...
def compare(before, after):
    for manifest in (before, after):
        if manifest.get("schema_version") != 1 or manifest.get("status") != "passed":
            raise ValueError(
                "comparison requires two successful schema-version-1 experiments"
            )
    if (
        before["pipeline"] != after["pipeline"]
        or before["heuristic"] != after["heuristic"]
    ):
        raise ValueError("pipeline and candidate heuristic must match")
    if (
        before["source_hashes"]["src/LPTAInstrumentation.cpp"]
        != after["source_hashes"]["src/LPTAInstrumentation.cpp"]
    ):
        raise ValueError(
            "plugin source differs; compiler-version effects cannot be isolated"
        )
    ...
    groups = [
        {(r["unit_kind"], r["pass"]): r for r in m["summary"]["by_pass_and_kind"]}
    ...
    return {
        "before_llvm": before["llvm_version"],
        "after_llvm": after["llvm_version"],
        ...
        "total_delta": {
            name: after["summary"][name] - before["summary"][name]
