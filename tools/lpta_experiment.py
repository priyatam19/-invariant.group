...
def read_trace(path):
    ...
    for seq, row in enumerate(rows):
        ...
        if not row["invalidated"]:
            for key in ("ir_changed", "cfg_changed", "before_instr_count", "after_instr_count"):
                if key not in row:
                    raise ValueError(f"{path}: missing {key} at event {seq}")
    return rows
...
def summarize(rows):
    ...
    for row in rows:
        ...
        group = groups.setdefault(key, {"unit_kind": key[0], "pass": key[1],
                                       "events": 0, "changed": 0, "candidates": 0})
        group["events"] += 1
    ...
    return {
        ...
        "incremental_update_candidates": sum(bool(r.get("incremental_update_candidate")) for r in rows),
...
def run_experiment(args):
    ...
    manifest = {"schema_version": 1, "status": "running", "commands": [], "checks": {},
    ...
    def run(command, overrides=None):
        ...
        result = subprocess.run(command, cwd=source, env={**env, **(overrides or {})},
                                text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        with (output / "commands.log").open("a") as log:
            log.write(json.dumps(manifest["commands"][-1]) + "\n" + result.stdout + result.stderr)
        if result.returncode:
            raise RuntimeError(f"command exited {result.returncode}: {command!r}\n{result.stderr}")
        return result.stdout.strip()

    try:
        tools = {name: prefix / "bin" / name for name in ("llvm-config", "clang", "opt")}
        ...
        manifest.update(llvm_version=version, tool_versions=versions,
                        llvm_prefix=str(prefix), pipeline=args.pipeline)
        inputs = args.input.resolve() if args.input else source / "test/sample.c"
        manifest["input"] = {"path": str(inputs), "sha256": sha256(inputs)}
        manifest["source_hashes"] = {name: sha256(source / name) for name in (
            "CMakeLists.txt", "src/LPTAInstrumentation.cpp", "tools/lpta_report.py")}
        ...
        run(["cmake", "-S", source, "-B", build, f"-DLLVM_DIR={cmakedir}",
        ...
        if inputs.suffix == ".ll":
            ...
            run([tools["clang"], "-S", "-emit-llvm", "-O0", "-Xclang",
                 "-disable-O0-optnone", inputs, "-o", ir])
        ...
        for name, flags in (("plain", []), ("instrumented", [load])):
            run([*opt, *flags, "-verify-each", "-S", ir, "-o", output / f"{name}.ll"],
                {"LPTA_TRACE_OUT": str(output / "verified-trace.jsonl")})
        if (output / "plain.ll").read_bytes() != (output / "instrumented.ll").read_bytes():
            raise ValueError("instrumentation changed the optimized IR")
        manifest["checks"]["verify_each_and_identical_optimized_ir"] = True
        functions = sorted({r["unit_name"] for r in rows if r["unit_kind"] == "function"})
        ...
        run([sys.executable, source / "tools/lpta_report.py", trace, "-o", output / "report.html"])
        manifest["checks"]["html_generation"] = True
        manifest["artifacts"] = {name: sha256(output / name) for name in (
            "input.ll", "trace.jsonl", "plain.ll", "instrumented.ll", "report.html")}
    ...
    print(json.dumps({"manifest": str(output / "manifest.json"),
                      "llvm": version, "events": len(rows), "checks": manifest["checks"]}, indent=2))


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--llvm-prefix", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True, help="new directory for this experiment")
    parser.add_argument("--source", type=Path, default=Path(__file__).resolve().parents[1])
    parser.add_argument("--input", type=Path, help="C or existing LLVM IR; defaults to test/sample.c")
    parser.add_argument("--build-dir", type=Path, help="optional reusable build directory")
